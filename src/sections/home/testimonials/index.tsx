...
function TestimonialCard({ item }: { item: (typeof TESTIMONIALS)[0] }) {
  ...
  return (
    ...
    >
      ...
      {/* ── content ── */}
      <div className="relative z-20 p-8 flex flex-col gap-5 h-full">
        {/* title row */}
        <div className="flex items-start justify-between gap-4">
          <h3 className="text-white font-semibold text-[1.05rem] leading-snug flex-1">
            {item.company}
          </h3>
        ...
        {/* body text — cyan like screenshot */}
        <p
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <div className="flex flex-col gap-0.5 min-w-0">
            <span className="text-white font-bold text-[13px] uppercase tracking-wide leading-tight">
              {item.name}
            </span>
            <span className="text-white/45 text-[12px] leading-tight truncate">
              {item.role}
            </span>
          </div>
        </div>
...
export default function Testimonials() {
  ...
  // how many pairs: 0→[0,1], 1→[2,3], 2→[4,5]
  const pairs = Math.ceil(total / 2);

  const advance = useCallback(() => {
    setIndex((c) => (c + 1) % pairs);
  }, [pairs]);
  ...
  const left = TESTIMONIALS[(index * 2) % total];
  const right = TESTIMONIALS[(index * 2 + 1) % total];
